...
(function () {
            ...
            // Logging out of the admin console must actually clear the
            // admin flag — otherwise it lingers in localStorage and the
            // browser is still treated as an authenticated admin later
            // (e.g. on edit-course.html) even after clicking "Logout".
...
(function () {
            ...
            function youtubeEmbedUrl(url) {
                ...
                var ytMatch = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/|youtube\.com\/shorts\/|m\.youtube\.com\/watch\?v=)([\w-]{11})/);
            ...
            function videoEmbedHTML(url, style) {
                var embed = youtubeEmbedUrl(url);
                if (embed) {
                    return '<iframe src="' + embed + '" style="' + style + '" allow="autoplay; encrypted-media" allowfullscreen></iframe>';
                }
                if (isDirectVideoFile(url)) {
                    return '<video src="' + url + '" controls style="' + style + 'object-fit:cover;"></video>';
                }
                return '<iframe src="' + url + '" style="' + style + '" allow="autoplay; encrypted-media" allowfullscreen></iframe>';
            }
                        ...
                        '<div class="module-pane" data-pane="video">' +
                            (m.video.url
                                ? '<div class="video-placeholder" style="cursor:default;">' +
                                        ...
                                        '<div class="video-media-wrap" style="width:100%;height:100%;position:relative;z-index:1;">' + videoEmbedHTML(m.video.url, "width:100%;height:100%;border:0;border-radius:inherit;") + '</div>' +
...
(function () {
            ...
            function moduleYoutubeEmbedUrl(url) {
                var ytMatch = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/|youtube\.com\/shorts\/|m\.youtube\.com\/watch\?v=)([\w-]{11})/);
                return ytMatch ? 'https://www.youtube.com/embed/' + ytMatch[1] : '';
            }

            function photoPreviewHTML(url) {
                if (!url) return '<span class="field-hint">No image yet — paste a URL above to preview it here.</span>';
                return '<img src="' + escapeAttr(url) + '" alt="Photo preview" style="max-width:220px;max-height:130px;border-radius:6px;object-fit:cover;display:block;" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'block\';">' +
                    '<span class="field-hint" style="display:none;">Couldn\'t load that image URL.</span>';
            }

            function videoPreviewHTML(url) {
                if (!url) return '<span class="field-hint">No video yet — paste a link above to preview it here.</span>';
                var embed = moduleYoutubeEmbedUrl(url);
                if (embed) {
                    return '<iframe src="' + embed + '" style="width:220px;height:130px;border:0;border-radius:6px;" allow="autoplay; encrypted-media" allowfullscreen></iframe>';
                }
                if (/\.(mp4|webm|ogg)(\?.*)?$/i.test(url)) {
                    return '<video src="' + escapeAttr(url) + '" controls style="max-width:220px;max-height:130px;border-radius:6px;"></video>';
                }
                return '<span class="field-hint">That link doesn\'t look like a YouTube link or a direct .mp4/.webm/.ogg URL yet.</span>';
            }
                    ...
                    '<div class="module-editor-card" data-module-index="' + index + '">' +
                        ...
                        '<div class="form-row">' +
                            ...
                            '<div class="field">' +
                                ...
                                '<div class="mod-media-preview mod-photo-preview">' + photoPreviewHTML(m.photoUrl) + '</div>' +
                            '</div>' +
                        '</div>' +
                        '<div class="form-row">' +
                            '<div class="field">' +
                        ...
                        '<div class="field">' +
                            ...
                            '<div class="mod-media-preview mod-video-preview">' + videoPreviewHTML(m.videoUrl) + '</div>' +
                        '</div>' +
                        '<div class="field">' +
            ...
            function renderModulesEditor() {
                ...
                els.modulesEditor.querySelectorAll('.mod-photo-url').forEach(function (input) {
                    input.addEventListener('input', function () {
                        var card = input.closest('.module-editor-card');
                        card.querySelector('.mod-photo-preview').innerHTML = photoPreviewHTML(input.value.trim());
                    });
                });
                els.modulesEditor.querySelectorAll('.mod-video-url').forEach(function (input) {
                    input.addEventListener('input', function () {
                        var card = input.closest('.module-editor-card');
                        card.querySelector('.mod-video-preview').innerHTML = videoPreviewHTML(input.value.trim());
                    });
                });
            }
...
/* ---------- student-login ----------
   Superseded by the ES module at js/pages/student-login.js, which
   student-login.html now loads directly (see auth.js / api.js / ui.js /
   validation.js). Left out of EduTrack.pages intentionally so app.js
   doesn't double-handle the form on that page. */
